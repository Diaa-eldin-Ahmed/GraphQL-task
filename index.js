...
const posts = [
    ...
    },
    {
        ...
        title: "City of Glass",
        text: "Lorem Ipsum has been the industry's standard dummy",
        comments: [{ name: "Diaa", content: "Give it 5" }],
    },
];
...
const resolvers = {
    ...
    Mutation: {
        ...
        updatePost: (_, { id, title, text }) => {
            let postToBeUpdatedIndex = posts.findIndex(
                (post) => post.id === id
            );
        ...
        deletePost: (_, { id }) => {
            let postsAfterDeleting = posts.filter((post) => post.id !== id);
            let postToBeDeletedIndex = posts.findIndex(
                (post) => post.id === id
            );
        ...
        addComment: (_, { postId, name, content }) => {
            let postToBeCommentedIndex = posts.findIndex(
                (post) => post.id === postId
            );
